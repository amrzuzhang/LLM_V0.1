...
# ----------------------------------------------------------------------------------------------------------------------------              
    ...
    mask = np.isfinite(y_true) & np.isfinite(y_pred_ens)
    ...
    for i in range(y_true.shape[0]):
        obs = np.squeeze(y_true[i,:]).reshape(-1)
        pre = np.squeeze(y_pred_ens[i,:]).reshape(-1)
        valid = np.isfinite(obs) & np.isfinite(pre)
        if np.count_nonzero(valid) < 2:
            R[i] = np.nan
            continue
        R[i] = np.corrcoef(obs[valid],pre[valid])[0,1]
