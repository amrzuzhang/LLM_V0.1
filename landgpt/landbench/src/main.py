...
import torch
from path_utils import resolve_root, resolve_mask_path
# ------------------------------------------------------------------------------ 
...
def _resolve_outputs_root(base: str, product: str, resolution: int) -> Path:
    return resolve_root(base, product, resolution)
...
def main(cfg):
    ...
    path_root = _resolve_outputs_root(cfg['inputs_path'], cfg['product'], cfg['spatial_resolution'])
    path_root.mkdir(parents=True, exist_ok=True)
    path = path_root.as_posix().rstrip('/') + '/'
    ...
    if os.path.exists(path+'/x_train_norm.npy'):
        ...
        mask = np.load(resolve_mask_path(path_root, cfg['spatial_resolution']))
...
# ------------------------------------------------------------------------------------------------------------------------------
    #Model training
    out_dir = path_root / cfg['workname'] / cfg['modelname'] / ('focast_time ' + str(cfg['forcast_time']))
    out_dir.mkdir(parents=True, exist_ok=True)
    model_ckpt = out_dir / (cfg['modelname'] + '_para.pkl')
    if model_ckpt.exists():
        print('[ATAI {d_p} work ] loading trained model'.format(d_p=cfg['workname'])) 
        model = _load_checkpoint(model_ckpt, device)
    else:
        # train 
        print('[ATAI {d_p} work ] training {m_n} model'.format(d_p=cfg['workname'],m_n=cfg['modelname'])) 
        for j in range(cfg["num_repeat"]):
            train(x_train, y_train, static, mask, scaler_x, scaler_y, cfg, j,path,out_dir.as_posix().rstrip('/')+'/',device)
            model = _load_checkpoint(model_ckpt, device)
...
# save predicted values and true values
    ...
    np.save(out_dir.as_posix().rstrip('/') +'/_predictions.npy', y_pred)
    np.save(out_dir.as_posix().rstrip('/') + '/observations.npy', y_test)
