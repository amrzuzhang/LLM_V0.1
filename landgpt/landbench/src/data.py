...
class Dataset():
    ...
    def __init__(self, cfg: dict):
        ...
        self.resolution_label = resolution_dirname(cfg['spatial_resolution'])
    ...
    def fit(self,cfg):
        ...
        PATH = self._resolve_path(self.inputs_path, self.product, self.s_resolution)
        data_path = self._resolve_path(cfg['nc_data_path'], self.product, self.s_resolution)
        print(PATH)
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#Load forcing data
        ...
        for year in range(begin_year, end_year+1):
            file_name_forcing = 'ERA5-Land_forcing {sr} spatial resolution {year}.npy'.format(sr=self.resolution_label,year=year)
        ...
        lat_file_name = lat_filename(self.s_resolution)
        lon_file_name = lon_filename(self.s_resolution)
	...
	#Memmap is to deal with the problem of excessive amount of data             
        ...
        forcing = self._temporal_fillna(forcing)
...
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#Load land_surface data
        land_surface_list = []
        for year in range(begin_year, end_year+1):
            file_name_land_surface = 'ERA5-Land_land_surface {sr} spatial resolution {year}.npy'.format(sr=self.resolution_label,year=year)
        ...
        land_surface = self._temporal_fillna(land_surface)
...
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#The default label is volumetric_soil_water_layer_20
        label = []
        for year in range(begin_year, end_year + 1):
            file_name_label = 'ERA5_LAND_label_{sr}_{year}.npy'.format(sr=self.resolution_label,year=year)
            ...
            data = self._temporal_fillna(data)
            label.append(data)
        ...
        print('label type is {d_p}'.format(d_p=label.dtype))
        mask_file = mask_filename(self.s_resolution)
        mask_bool = self._build_spatial_mask(forcing, land_surface, label)
        if not np.any(mask_bool):
            raise RuntimeError("All grid cells became invalid during preprocessing; check data sources.")
        print("mask shape is ",mask_bool.shape, "valid points:", int(mask_bool.sum()))
        mask = mask_bool.astype(np.uint8)
        np.save(PATH + mask_file, np.squeeze(mask))
        mask = np.load(PATH + mask_file)
        mask_bool = mask.astype(bool)
        label = np.where(mask_bool[None, :, :, None], label, np.nan)
# ------------------------------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------------------------------
	#Partition and create datasets
        ...
        N = int(365 * len(self.test_year) + cfg['seq_len'] + cfg['forcast_time'])
        train_samples = int(max(0, self.time_length_f - N))
	...
	#Create a training dataset；The default training data is 1990 to 2019.
        if not os.path.exists(PATH+'x_train.npy'):
            x_train = np.memmap(PATH+'x_train.npy',dtype=cfg['data_type'],mode='w+',shape=(train_samples, self.nlat_f, self.nlon_f, self.num_features_f+self.num_features_l))
            x_train[:,:,:,:self.num_features_f] = forcing[:train_samples]
            x_train[:,:,:,self.num_features_f:] = land_surface[:train_samples]
            x_train.flush()
            del x_train
        x_train = np.memmap(PATH+'x_train.npy',dtype=cfg['data_type'],mode='r+',shape=(train_samples, self.nlat_f, self.nlon_f, self.num_features_f+self.num_features_l))
...
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#Create a test dataset ；The default for the test dataset is only 2020
        if not os.path.exists(PATH+'x_test.npy'):
            ...
            x_test[:,:,:,:self.num_features_f],x_test[:,:,:,self.num_features_f:] = forcing[train_samples:],land_surface[train_samples:]
            x_test.flush()
            del x_test
        ...
        print('[ATAI {d_p} work ] finish  generating y_test data, y_test shape is {yts}'.format(d_p=cfg['workname'],yts=y_test.shape))
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def reverse_normalize(
        ...
        return feature
#------------------------------------------------------------------------------------------------------------------------------
    def _resolve_path(self, base: str, product: str, resolution: int) -> str:
        """Gracefully resolve base/product/resolution directories."""

        resolved = resolve_root(base, product, resolution)
        return resolved.as_posix().rstrip('/') + '/'
#------------------------------------------------------------------------------------------------------------------------------
    def _build_spatial_mask(self, forcing, land_surface, label):
        """Return boolean mask that keeps pixels with any finite samples."""

        label_valid = np.any(np.isfinite(label), axis=(0, 3))
        forcing_valid = np.any(np.isfinite(forcing), axis=(0, 3))
        land_valid = np.any(np.isfinite(land_surface), axis=(0, 3))
        return label_valid & forcing_valid & land_valid
#------------------------------------------------------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def _temporal_fillna(self, arr):
            """Fill NaNs along the time axis via forward/backward fill and fallback to zeros."""
            ...
            forward = flat[idx, np.arange(flat.shape[1])]
            forward[idx == 0] = np.nan
            # backward fill
            mask_rev = mask[::-1]
            idx_rev = np.where(mask_rev, np.arange(t)[:, None], 0)
            np.maximum.accumulate(idx_rev, axis=0, out=idx_rev)
            backward = flat[::-1][idx_rev, np.arange(flat.shape[1])]
            backward[idx_rev == 0] = np.nan
            filled = forward
            need_backward = ~np.isfinite(filled)
            filled[need_backward] = backward[::-1][need_backward]
            still_nan = ~np.isfinite(filled)
            if np.any(still_nan):
                filled[still_nan] = 0.0
            return filled.reshape(t, h, w, c)
