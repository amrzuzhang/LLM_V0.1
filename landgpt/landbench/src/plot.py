...
out_path_lstm = (work_root / 'LSTM' / focast_dir).as_posix().rstrip('/') + '/'
out_path_CNN = (work_root / 'CNN' / focast_dir).as_posix().rstrip('/') + '/'
out_path_convlstm = (work_root / 'ConvLSTM' / focast_dir).as_posix().rstrip('/') + '/'
mask = np.load(resolve_mask_path(base_path, cfg['spatial_resolution']))

required_predictions = {
    'LSTM': out_path_lstm + '_predictions.npy',
    'CNN': out_path_CNN + '_predictions.npy',
    'ConvLSTM': out_path_convlstm + '_predictions.npy',
}
missing_models = [name for name, path in required_predictions.items() if not os.path.exists(path)]
if missing_models:
    print('[plot.py] missing predictions for: {}.'.format(', '.join(missing_models)))
    print('[plot.py] This script compares LSTM/CNN/ConvLSTM. Train the missing models or use plot_test.py for single-model plots.')
    sys.exit(0)
...
lon_ = np.load(base_path / lon_filename(cfg['spatial_resolution']))
lat_ = np.load(base_path / lat_filename(cfg['spatial_resolution']))
